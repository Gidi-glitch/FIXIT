...
const TRADESMEN_DATA: Tradesman[] = [
  { id: "t1", initials: "MR", color: "linear-gradient(135deg,#1560B0,#2E82D8)", name: "Marco Reyes",      email: "marco.r@gmail.com",   category: "⚡ Electrician",     license: "ELEC-2024-00847", joined: "Jan 2025", jobs: 0,  status: "Pending"  },
  { id: "t2", initials: "JC", color: "linear-gradient(135deg,#0F7060,#17A88E)", name: "Jake Cruz",        email: "jakecruz@gmail.com",  category: "🔧 Plumber",         license: "PLMB-2023-11204", joined: "Feb 2025", jobs: 0,  status: "Pending"  },
  { id: "t3", initials: "AL", color: "linear-gradient(135deg,#5B2D8E,#8040C0)", name: "Ana Lim",          email: "ana.lim@gmail.com",   category: "❄️ HVAC Technician", license: "HVAC-2024-05563", joined: "Mar 2025", jobs: 0,  status: "Pending"  },
  { id: "t4", initials: "RD", color: "linear-gradient(135deg,#1B2B5E,#2D44A0)", name: "Ramon Dela Cruz",  email: "ramon.dc@gmail.com",  category: "🔌 Appliance Repair",license: "APPL-2022-00312", joined: "Nov 2024", jobs: 34, status: "Verified" },
  { id: "t5", initials: "PV", color: "linear-gradient(135deg,#1A7A4A,#26A864)", name: "Pedro Villarta",   email: "pedro.v@yahoo.com",   category: "🎨 Painter",         license: "PAINT-2023-07791",joined: "Oct 2024", jobs: 21, status: "Verified" },
];

const HOMEOWNERS_DATA: Homeowner[] = [
  { id: "h1", initials: "SM", color: "linear-gradient(135deg,#A82040,#D03060)", name: "Sofia Mendoza",  email: "sofia.m@gmail.com",       location: "Quezon City",  registered: "Feb 12, 2025", jobs: 4,  status: "Active"   },
  { id: "h2", initials: "BT", color: "linear-gradient(135deg,#1560B0,#2E82D8)", name: "Ben Torres",     email: "bentorres@yahoo.com",     location: "Makati",       registered: "Jan 4, 2025",  jobs: 7,  status: "Active"   },
  { id: "h3", initials: "LV", color: "linear-gradient(135deg,#B85010,#E87722)", name: "Liza Villanueva",email: "liza.v@outlook.com",       location: "Pasig City",   registered: "Mar 1, 2025",  jobs: 2,  status: "Active"   },
  { id: "h4", initials: "KS", color: "linear-gradient(135deg,#0F7060,#17A88E)", name: "Karl Santos",    email: "karlsantos99@gmail.com",   location: "Mandaluyong",  registered: "Dec 18, 2024", jobs: 11, status: "Active"   },
  { id: "h5", initials: "MG", color: "linear-gradient(135deg,#5B2D8E,#8040C0)", name: "Maria Garcia",   email: "maria.g@fixit.ph",         location: "Taguig",       registered: "Nov 5, 2024",  jobs: 6,  status: "Active"   },
  { id: "h6", initials: "JR", color: "linear-gradient(135deg,#1B2B5E,#2D44A0)", name: "Jose Ramos",     email: "jose.r@gmail.com",         location: "Quezon City",  registered: "Oct 22, 2024", jobs: 3,  status: "Inactive" },
];
...
const Td = ({ children, style = {} }: { children: ReactNode; style?: React.CSSProperties }) => (
  <td style={{ padding: "14px 20px", fontSize: 13, color: "#0F1923", borderBottom: "1px solid #E3E8F0", verticalAlign: "middle", ...style }}>
    {children}
  </td>
);
...
const Toolbar = ({ placeholder, filters }: { placeholder: string; filters: string[][] }) => (
  <div style={{ padding: "14px 24px", borderBottom: "1px solid #E3E8F0", background: "#F7F8FA", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
    <div style={{ flex: 1, minWidth: 200, display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", background: "#fff", border: "1.5px solid #E3E8F0", borderRadius: 8 }}>
      ...
      <input type="text" placeholder={placeholder} style={{ border: "none", outline: "none", background: "transparent", fontFamily: "inherit", fontSize: 13, color: "#0F1923", width: "100%" }} />
    </div>
    {filters.map((opts, i) => (
      <select key={i} defaultValue="" style={{ padding: "9px 14px", background: "#fff", border: "1.5px solid #E3E8F0", borderRadius: 8, fontFamily: "inherit", fontSize: 13, fontWeight: 600, color: "#4A5568", outline: "none", cursor: "pointer", minWidth: 140 }}>
...
export default function DashboardPage() {
  ...
  const [tradesmen, setTradesmen]   = useState<Tradesman[]>(TRADESMEN_DATA);
  const [homeowners]                = useState<Homeowner[]>(HOMEOWNERS_DATA);
  ...
  const pendingCount  = tradesmen.filter((t) => t.status === "Pending").length;
  ...
  // Approve / Reject
  const approveTradesman = (id: string, name: string) => {
  ...
  const openHOModal = (h: Homeowner) => {
    setModal({
      ...
      rows: [
        ...
        { label: "Status",    value: h.status, highlight: h.status === "Active" },
      ],
    });
  };
  ...
  const PageDashboard = () => (
    <div style={{ animation: "fadeUp .35s ease both" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 28 }}>
        <StatCard iconBg="#EEF1FA" iconColor="#1B2B5E" num={HOMEOWNERS_DATA.length} label="Total Homeowners" trend="+12%" trendType="up"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20, marginBottom: 28 }}>
        ...
        <Card>
          <CardHead
            ...
            subtitle="Tradesmen awaiting review"
            right={
              <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 16, padding: "20px 24px" }}>
            {tradesmen.filter((t) => t.status === "Pending").map((t) => (
              <VCard key={t.id} t={t} onApprove={() => approveTradesman(t.id, t.name)} onReject={() => rejectTradesman(t.id, t.name)} />
            ))}
            {pendingCount === 0 && (
        ...
        <Card>
          ...
          <ActivityItem dot="#1B2B5E" title="New homeowner signed up"  sub="Maria Garcia · Taguig"          time="1h ago" />
  ...
  const PageVerification = () => (
    <div style={{ animation: "fadeUp .35s ease both" }}>
      <Card>
        <CardHead title="Tradesman Verifications" subtitle="Review and approve license submissions"
          right={<Pill color="orange">{pendingCount} Pending</Pill>}
        />
        <Toolbar placeholder="Search by name or license…" filters={[["All Status","Pending","Verified"],["All Categories","Electrician","Plumber","HVAC","Carpenter","Painter"]]} />
        <Table>
          <thead><tr><Th>Tradesman</Th><Th>Category</Th><Th>License No.</Th><Th>Joined</Th><Th>Status</Th><Th>Actions</Th></tr></thead>
          <tbody>
            {tradesmen.map((t) => (
              <tr key={t.id}
                onMouseEnter={(e) => (e.currentTarget.style.background = "#FAFBFD")}
                ...
                <Td>{t.category}</Td>
                <Td>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#F7F8FA", border: "1px solid #E3E8F0", borderRadius: 6, padding: "5px 10px", fontSize: 12, fontWeight: 600, color: "#4A5568", fontFamily: "monospace" }}>
                    {icons.license}{t.license}
                  </span>
                </Td>
                <Td>{t.joined}</Td>
                ...
                <Td>
                  <div style={{ display: "flex", gap: 8 }}>
                    {t.status === "Pending" ? (
                      <>
                        <Btn variant="approve" onClick={() => approveTradesman(t.id, t.name)}>{icons.check} Approve</Btn>
                        <Btn variant="reject"  onClick={() => rejectTradesman(t.id, t.name)}>{icons.x} Reject</Btn>
                      </>
                    ) : (
                      <>
                        <Btn disabled>{icons.check} Approved</Btn>
                        <Btn variant="reject" onClick={() => showToast(`${t.name} revoked`, "error")}>{icons.x} Revoke</Btn>
                      </>
                    )}
                  </div>
                </Td>
  ...
  const PageTradesmen = () => (
    <div style={{ animation: "fadeUp .35s ease both" }}>
      <Card>
        ...
        <Toolbar placeholder="Search tradesmen…" filters={[["All Categories","Electrician","Plumber","HVAC","Carpenter","Painter"],["All Status","Verified","Pending"]]} />
        <Table>
          ...
          <tbody>
            {tradesmen.map((t) => (
              <tr key={t.id}
                ...
                <Td>
                  <div style={{ display: "flex", gap: 8 }}>
                    {t.status === "Pending" && <Btn variant="approve" onClick={() => approveTradesman(t.id, t.name)}>{icons.check} Approve</Btn>}
  ...
  const PageHomeowners = () => (
    <div style={{ animation: "fadeUp .35s ease both" }}>
      <Card>
        <CardHead title="All Homeowners" subtitle={`${homeowners.length} registered homeowners`} right={<Pill color="navy">{homeowners.length} Total</Pill>} />
        <Toolbar placeholder="Search homeowners…" filters={[["All Locations","Quezon City","Makati","Pasig","Taguig","Mandaluyong"],["All Status","Active","Inactive"]]} />
        <Table>
          <thead><tr><Th>Homeowner</Th><Th>Location</Th><Th>Registered</Th><Th>Jobs Posted</Th><Th>Status</Th><Th>Actions</Th></tr></thead>
          <tbody>
            {homeowners.map((h) => (
              <tr key={h.id}
                onMouseEnter={(e) => (e.currentTarget.style.background = "#FAFBFD")}
      ...
      <div style={{ marginLeft: 260, flex: 1, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        <div style={{ background: "#fff", height: 64, borderBottom: "1px solid #E3E8F0", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", position: "sticky", top: 0, zIndex: 40, boxShadow: "0 1px 3px rgba(15,25,35,.06)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {/* Search */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", background: "#F7F8FA", border: "1.5px solid #E3E8F0", borderRadius: 8, width: 220, color: "#9AA3B8", fontSize: 13, cursor: "text" }}>
