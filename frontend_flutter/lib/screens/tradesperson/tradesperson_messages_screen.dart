...
class TradespersonMessagesScreen extends StatefulWidget {
  ...
  final int? initialHomeownerUserId;
...
class _TradespersonMessagesScreenState extends State<TradespersonMessagesScreen>
  ...
  Future<Map<String, dynamic>> _ensureConversationFromBackend({
    ...
    final otherUserId = sourceOtherUserId > 0
        ? sourceOtherUserId
        : (widget.initialHomeownerUserId ?? 0);
    ...
    final normalized = _normalizeConversation(
      rawConversation,
      fallbackService: (widget.initialService ?? '').trim().isNotEmpty
  ...
  Future<void> _openChatForInitialHomeowner() async {
    ...
    var targetConversation = _findConversationByHomeowner(name);
  ...
  Future<void> _openConversationChat(Map<String, dynamic> conversation) async {
    ...
    final result = await Navigator.push<Map<String, dynamic>>(
      context,
      MaterialPageRoute(
        builder: (_) => TradespersonChatScreen(conversation: readyConversation),
      ),
    );
