...
func ConversationsRoot(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		ensureConversationForUser(w, r, user)
		return
	}
	...
	response := buildConversationResponseRows(r, user, conversations)

	writeJSON(w, http.StatusOK, map[string]any{"conversations": response})
}

func ensureConversationForUser(
	...
	otherUserID := uint(0)
	if req.OtherUserID != nil && *req.OtherUserID != 0 {
		otherUserID = *req.OtherUserID
	}

	if otherUserID == 0 && req.BookingID != nil && *req.BookingID != 0 {
		booking, ok := getBookingPairByID(w, *req.BookingID)
		if !ok {
			return
		}
		...
		if user.Role == "homeowner" {
			if booking.HomeownerID != user.ID {
				writeError(w, http.StatusForbidden, "booking is not accessible")
				return
			}
			otherUserID = booking.TradespersonID
		} else {
			if booking.TradespersonID != user.ID {
				writeError(w, http.StatusForbidden, "booking is not accessible")
				return
			}
			otherUserID = booking.HomeownerID
		}
	}

	if otherUserID == 0 {
		resolvedID, err := resolveCounterpartyUserIDByName(targetRole, req.OtherUserName)
		if err != nil {
	...
	if otherUserID == 0 || otherUserID == user.ID {
		writeError(w, http.StatusBadRequest, "invalid counterparty user")
		return
	}

	if err := ensureCounterpartyRole(otherUserID, targetRole); err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			writeError(w, http.StatusNotFound, "counterparty not found")
			return
		}
		writeError(w, http.StatusInternalServerError, "failed to validate counterparty")
		return
	}
	...
	if req.BookingID != nil && *req.BookingID != 0 {
		...
		if booking.HomeownerID != homeownerID || booking.TradespersonID != tradespersonID {
			writeError(w, http.StatusBadRequest, "booking does not belong to this conversation pair")
			return
		}
		idCopy := booking.ID
		bookingID = &idCopy
	...
	conversation, err := getOrCreateConversation(homeownerID, tradespersonID, bookingID)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to prepare conversation")
		return
	}

	if err := unhideConversationForUser(conversation.ID, user.Role); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to prepare conversation visibility")
		return
	}

	if err := config.DB.Where("id = ?", conversation.ID).First(&conversation).Error; err != nil {
		writeError(w, http.StatusInternalServerError, "failed to load conversation")
		return
	}

	rows := buildConversationResponseRows(r, user, []models.Conversation{conversation})
	if len(rows) == 0 {
		writeError(w, http.StatusInternalServerError, "failed to build conversation response")
		return
	}
...
func getBookingPairByID(w http.ResponseWriter, bookingID uint) (models.Booking, bool) {
	var booking models.Booking
	if err := config.DB.Select("id", "homeowner_id", "tradesperson_id").
		Where("id = ?", bookingID).
...
func ensureCounterpartyRole(userID uint, role string) error {
	...
	if strings.TrimSpace(strings.ToLower(user.Role)) != strings.TrimSpace(strings.ToLower(role)) {
		return gorm.ErrRecordNotFound
	}
...
func resolveCounterpartyUserIDByName(role string, rawName string) (uint, error) {
	normalized := strings.ToLower(strings.TrimSpace(rawName))
	if normalized == "" {
		return 0, errors.New("otherUserId, bookingId, or otherUserName is required")
	}

	parts := strings.Fields(normalized)
	if len(parts) < 2 {
		return 0, errors.New("otherUserName must include first and last name")
	}

	if role == "tradesperson" {
		var profiles []models.TradespersonProfile
		if err := config.DB.Select("user_id").
			Where("LOWER(TRIM(first_name || ' ' || last_name)) = ?", normalized).
			Order("user_id ASC").
			Limit(2).
		...
		if len(profiles) == 0 {
			return 0, errors.New("counterparty not found")
		}
		if len(profiles) > 1 {
			return 0, errors.New("multiple users matched the provided otherUserName")
		}
...
func buildConversationResponseRows(
	r *http.Request,
	user messagingUser,
...
func unhideConversationForUser(conversationID uint, role string) error {
	updates := map[string]any{}
	if role == "homeowner" {
		updates["homeowner_deleted_at"] = nil
		updates["homeowner_archived_at"] = nil
	} else {
		updates["tradesperson_deleted_at"] = nil
